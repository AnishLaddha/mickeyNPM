...
async function runTests() {
    ...
    await runTest('fetchRepoInfoValid', async () => {
        const result = await fetch_repo_info('https://github.com/octocat/Hello-World');
    ...
    await runTest('fetchRepoInfoNpmUrl', async () => {
        const result = await fetch_repo_info('https://www.npmjs.com/package/lodash');
        return result.owner !== undefined && result.name !== undefined;
    });
    ...
    await runTest('calculateRampUpMetricValid', async () => {
        ...
        return result.RampUp >= 0 && result.RampUp <= 1 && result.RampUp_Latency > 0;
    });
    ...
    await runTest('calculateCorrectnessMetricValid', async () => {
        ...
        return result.Correctness >= 0 && result.Correctness <= 1 && result.Correctness_Latency > 0;
    });
    ...
    await runTest('calculateResponsivenessMetricValid', async () => {
        const result = await calculate_responsiveness_metric('socketio', 'socket.io');
        return result.ResponsiveMaintainer >= 0 && result.ResponsiveMaintainer <= 1 && result.ResponsiveMaintainer_Latency > 0;
    });
    ...
    await runTest('calculateLicenseMetricLGPLCompatible', async () => {
        ...
        return result.License >= 0 && result.License <= 1 && result.License_Latency > 0;
    });
    ...
    await runTest('fullMetricCalculation', async () => {
        const { owner, name } = await fetch_repo_info('https://github.com/Unitech/pm2');
        ...
        const netScore = calculate_net_score(license.License, rampUp.RampUp, correctness.Correctness, responsiveness.ResponsiveMaintainer);
        return netScore.NetScore >= 0 && netScore.NetScore <= 1;
    });

    // Integration test 2 (E2E test)
    await runTest('fullMetricCalculation2', async () => {
        const { owner, name } = await fetch_repo_info('https://www.npmjs.com/package/axios');
        const license = await calculate_license_metric(owner, name);
        const rampUp = await calculate_rampup_metric(owner, name);
        const correctness = await calculate_correctness_metric(owner, name);
        const responsiveness = await calculate_responsiveness_metric(owner, name);
    ...
    console.log(`${passedTests}/${totalTests} test cases passed. ${coverage.toFixed(2)}% line coverage achieved.`);
}
